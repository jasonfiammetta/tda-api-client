...
const tdaapiclient = require('../../index');
...
export default class StreamData extends EventEmitter {
    ...
    constructor(streamConfig: IStreamConfig) {
        ...
        this.defaultFields = StreamData.setDefaultFields();
        ...
        this.emitDataBySubAndTickerTyped = streamConfig.emitDataBySubAndTickerTyped || false;
    ...
    setEmitGranularity(config: IStreamConfig) : void {
    ...
    getEmitGranularity() : IStreamConfig {
        return {
    ...
    handleParamStorage(config: GenericStreamConfig) {
    ...
    async doDataStreamLogin(
        ...
        this.userPrincipalsResponse = await tdaapiclient.userinfo.getUserPrincipals({fields: fields, authConfig: this.authConfig});
    ...
    async accountUpdatesSub(accountIds: string = '', fields: string = "0,1,2,3", requestSeqNum: number = this.requestId++) : Promise<number> {
        if (accountIds === null || accountIds === '') {
            const allAccounts = await tdaapiclient.accounts.getAccounts();
        ...
        const streamKeyObj = await tdaapiclient.userinfo.getStreamerSubKeys({
